...
// The interactive TUI (charm.land/*), syntax highlighter (chroma, x/ansi), MCP
// SDK (modelcontextprotocol/go-sdk) and pi catalog (sky-valley/pi) are excluded
// from the win7 build by //go:build win7 tags, so they are intentionally NOT
// required here: pruning them keeps this modfile honest about what the Win7
// binary actually links. `go list -modfile=go.win7.mod -tags win7 -deps
// ./cmd/reasonix` confirms none of them enter the win7 build graph.
require (
