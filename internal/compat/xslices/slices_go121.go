...
// Package xslices re-exports the slice helpers Reasonix uses. On the native
// toolchain the wrappers forward to the standard slices package; under the
// go1.20.14 Win7 toolchain they forward to golang.org/x/exp/slices, which lacks
// the Go 1.23 iterator helpers (Backward, Sorted). Those iterator helpers are
// only provided on the native side; call sites that use them are excluded from
